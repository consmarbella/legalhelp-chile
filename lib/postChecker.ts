...
export function checkDocument(doc: string, tipo: string): CheckResult {
  ...
  if (pendientes > 0) {
    errors.push(`ESTA PROHIBIDO USAR [DATO PENDIENTE] (${pendientes} encontrados) - reemplázalos por "no especificado" o adapta el texto.`);
  }
  ...
  // Todos los placeholders genéricos están prohibidos
  const realGeneric = genericPlaceholders;
