...
function PagoPendienteInner() {
  ...
  const [estado, setEstado] = useState<'verificando' | 'aprobado' | 'pendiente'>('verificando');
  const [intentos, setIntentos] = useState(0);
  const MAX_INTENTOS = 20; // 20 × 3s = 60 segundos máximo
  const timerRef = useRef<ReturnType<typeof setTimeout> | null>(null);

  useEffect(() => {
    if (!orderId) { setEstado('pendiente'); return; }
    ...
    const verificar = async () => {
      try {
        ...
        if (data.status === 'approved') {
          ...
          // Sigue pendiente: algunos medios (transferencia, efectivo) tardan más
          setEstado('pendiente');
        }
      } catch {
    ...
    <div className="min-h-screen bg-[#0b1f3a] flex items-center justify-center p-6">
      <div className="bg-white rounded-2xl shadow-2xl max-w-sm w-full p-8 text-center" style={{ fontFamily: 'sans-serif' }}>
        ...
        {estado === 'pendiente' && (
          <>
            <div className="text-5xl mb-4">⏳</div>
            <h1 className="text-xl font-bold text-[#0b1f3a] mb-2">Pago pendiente</h1>
            <p className="text-sm text-[#8a7f72] mb-4">
              Tu pago está en proceso. Algunos medios (transferencia o efectivo) pueden
              tardar en confirmarse. Apenas se acredite podrás generar tu documento.
              {orderId && (
                <>
                  <br />Guarda tu número de orden:<br />
