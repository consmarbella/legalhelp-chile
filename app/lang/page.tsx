...
export default function LangPage() {
  ...
  const statusText = paid
    ...
    : 'SISTEMA EN LÍNEA · AGENTE LANGGRAPH ACTIVO';
  ...
  return (
    <div className="min-h-screen text-white">
      ...
      <header className="pt-16 pb-12">
        <div className="max-w-3xl mx-auto px-6 text-center">
          ...
          <h1 className="text-4xl sm:text-6xl font-bold leading-[1.05] tracking-tight mb-5 lg-glow-title">
            Sistema LangGraph + RAG<br />
            <span className="text-cyan lg-glow-cyan">Agente Conversacional Legal</span>
          </h1>
          <p className="text-white text-base sm:text-lg leading-relaxed max-w-2xl mx-auto">
            Versión experimental con LangGraph + RAG. El agente usa herramientas (tools) para 
            consultar requisitos legales, validar datos y decidir cuándo generar el documento.
          </p>
          <div className="flex flex-wrap justify-center gap-2.5 mt-7">
            {['🤖 Agente LangGraph', '🧠 RAG + Vectorstore', '🔧 Tool Calling'].map(b => (
          ...
          <div className="glass-panel rounded-2xl overflow-hidden flex flex-col">
            <div className="flex items-center justify-between px-4 py-2.5 border-b border-[#60a5fa]/15 bg-[#0d1426]/60">
              <div className="flex items-center gap-2">
                ...
                <span className="hud-label text-[#9ab0cc] ml-2">agente_langgraph.exe</span>
              </div>
              <div className="flex items-center gap-1.5">
                <span className="lg-status-dot" />
                <span className="hud-label text-cyan">GRAPH ACTIVO</span>
              </div>
            </div>
              ...
              {messages.length === 0 && (
                <div className="h-full flex items-center justify-center">
                  <p className="text-[#9ab0cc] text-sm text-center">
                    ...
                    <span className="text-xs text-[#7a90aa]">El agente usará tools para consultar requisitos y validar datos.</span>
                  </p>
                </div>
              )}
              {messages.map((m, i) => (
      ...
      {/* ── FOOTER ──────────────────────────────────────────────────── */}
      <footer className="border-t border-[#60a5fa]/15 py-5 mt-10">
        <div className="max-w-4xl mx-auto px-6 text-center text-xs text-[#7a90aa]">
          <p className="mb-2">🧪 Versión experimental — LangGraph + RAG + Tool Calling</p>
          <p>Compara con <a href="/" className="text-cyan hover:underline">la versión estándar</a></p>
        </div>
      </footer>
