...
export default function LangHome() {
  ...
  const handlePayment = async (plan: 'single' | 'monthly') => {
    ...
    try {
      const res = await fetch('/api/payment/create', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ plan, caseData, docId: selectedDoc, returnPath: '/lang' }),
      });
      const data = await res.json();
  ...
  const handleDownload = async () => {
    ...
    const nombreStr = String((caseData.datos as Record<string, unknown>)?.nombre ?? caseData.nombre ?? 'documento');
  ...
  const statusText = paid
    ...
    : 'MOTOR LANGGRAPH · 5 NODOS ACTIVOS';
