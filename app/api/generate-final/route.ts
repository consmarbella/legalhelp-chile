...
async function generateDocument(
  ...
  const frameworkBlock = template
    ? `\n\nFRAMEWORK LEGAL VERIFICADO PARA ESTE CASO (${template.titulo} — tipo ${template.tipo}):\n` +
      ...
      `ESTRUCTURA BASE DEL DOCUMENTO (respeta este esqueleto; rellena los [[...]] con los hechos del caso. ESTA PROHIBIDO DEJAR ESPACIOS EN BLANCO O CORCHETES COMO [DATO], ajusta la redaccion si algo falta):\n${template.esqueleto}\n\n` +
  ...
  // Llamada directa a Gemini (sin Agentic Swarm)
  console.log(`[generate-final] Generando con Gemini: ${tipo}`);
  const documentoCompleto = await llmComplete({
    system: SYSTEM,
    messages: [{
      role: 'user',
      content: `Redacta el siguiente documento legal chileno.\n\nTIPO: ${tipo}\nDATOS DEL CASO:\n${datosStr}\n\n${frameworkBlock}\n${groundingBlock}\n${keyDataSection}\n\nIMPORTANTE: Redacta el documento COMPLETO en texto plano (sin markdown, sin asteriscos). Usa SOLO los datos proporcionados, sin inventar nada.`
    }],
    maxTokens: 8192,
    temperature: 0.2,
  });
  return documentoCompleto ?? buildMock(tipo, datos);
}
...
function truncateForPreview(doc: string): string {
  ...
  // Mostrar 60% del documento: ven bastante, falta lo justo para motivar el pago
  const keep = Math.max(10, Math.ceil(lines.length * 0.6));
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    console.log(`[generate-final] Generating "${tipo}" with Swarm (paid=${isPaid})`);
    ...
    if (!check.passed) {
      ...
      // Reintentar UNA vez con prompt de corrección usando llmComplete (fallback fallback)
