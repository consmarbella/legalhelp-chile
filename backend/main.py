"""
LegalHelp TAG Core Backend
============================
Servidor FastAPI que utiliza PydanticAI + DeepSeek para extraer
multas TAG desde texto de certificados, calcular prescripción
(3 años desde 2026) y agrupar por Juzgado de Policía Local.
...
class MultaExtract(BaseModel):
    ...
    tribunal: str = Field(description="Nombre del Juzgado de Policía Local de origen, ej: '1° Juzgado de Policía Local de Santiago'")
    rol: str = Field(description="Número identificador del Rol o Causa, ej: '12345-2022'")
...
agente_tag = Agent(
    ...
    system_prompt=(
        ...
        "Ej: '1 POLICIA LOCAL DE SANTIAGO' → '1 POLICIA LOCAL DE SANTIAGO'. NO traduzcas ni modifiques.\n"
...
async def parse_tag(payload: ParseTagRequest):
    """
    Recibe texto plano extraído de un certificado de multas PDF,
    lo procesa con PydanticAI + DeepSeek y devuelve las multas
    agrupadas por tribunal, filtrando solo aquellas con más de 3 años
    de antigüedad desde la fecha actual (2026).
    """
    ...
    # --- 1. Ejecutar el agente PydanticAI ---
    try:
    ...
    # --- 2. Calcular cutoff de 3 años ---
    hoy = datetime.now()
    cutoff = hoy - relativedelta(years=3)

    # --- 3. Filtrar y agrupar por tribunal ---
    groups: Dict[str, List[MultaOut]] = {}
    ...
    for m in datos.multas:
        try:
            ...
            # Si la fecha no se puede parsear, omitimos esta multa
            continue

        # Solo multas con más de 3 años de antigüedad
        if f_ingreso <= cutoff:
